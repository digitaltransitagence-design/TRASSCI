 ...
 * @param {{ price?: number }} [options] — si `price` est fourni, évite un second chargement des règles Insforge
 */
export async function insertPackageAndHistory(payload, options = {}) {
  const price =
    typeof options.price === "number" && !Number.isNaN(options.price)
      ? options.price
