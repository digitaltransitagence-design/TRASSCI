...
export function computePrice(body) {
  const dest = TARIFS.destinations[body.destination] ?? 0;
  const mode =
    body.delivery_mode === "ramassage"
      ? TARIFS.options.ramassage
      : TARIFS.options.depot;
  const ins = body.has_insurance ? TARIFS.options.assurance : 0;
...
export async function insertPackageAndHistory(payload) {
  const price = computePrice(payload);
  const row = {
    id: payload.id,
