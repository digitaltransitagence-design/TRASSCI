...
/** Resend exige : `email@domaine` ou `Nom <email@domaine>` (pas de guillemets parasites). */
const DEFAULT_RESEND_FROM = "Trass CI <onboarding@resend.dev>";
...
export function normalizeResendFrom(raw) {
  ...
  if (bracket) {
    ...
    if (isValidEmailPart(email) && name.length > 0) {
      return `${name} <${email}>`;
    }
  }
...
export async function sendClientGuideEmail({ to }) {
  ...
  const replyTo =
    replyToRaw && isValidEmailPart(replyToRaw) ? replyToRaw : undefined;
